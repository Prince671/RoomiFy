...
const ChatRoom = ({ roomId }) => {
    ...
    const searchYouTube = async (query) => {
        ...
        try {
            ...
            setSearchResults(data.items); // ✅ Update state to display results
        } catch (error) {
    ...
    modal && (
        <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-80 z-50">
            ...
            <div className="mb-4 flex flex-col items-center space-y-2">
                ...
                <div className="mt-4 w-full max-w-md bg-gray-800 p-4 rounded-lg">
                    {searchResults.length === 0 ? (
                        <p className="text-white text-center">No results found</p>
                    ) : (
                        searchResults.map((video) => (
                            <div key={video.id.videoId} className="flex items-center justify-between mb-2 bg-gray-700 p-2 rounded-lg">
                                <div className="flex items-center space-x-4">
                                    <img src={video.snippet.thumbnails.default.url} alt={video.snippet.title} className="w-16 h-16 rounded-lg" />
                                    <span className="text-white">{video.snippet.title}</span>
                                </div>
                                <button
